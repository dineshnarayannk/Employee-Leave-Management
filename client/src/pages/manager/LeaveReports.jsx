...
import { getManagerReports, getLeaveTypes, getTeamBalances } from '../../services/api';
import {
  FileSpreadsheet,
  Download,
  Printer,
...
export default function LeaveReports() {
  ...
  const fetchReports = async () => {
    try {
      ...
      setError(err.message || 'Failed to generate manager leave report.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleExportCSV = () => {
    ...
    link.href = url;
    link.setAttribute('download', `team-leave-report-${year || 'all'}.csv`);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900 border border-slate-800 rounded-2xl p-6 relative overflow-hidden print:border-none print:p-0">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            Comprehensive leave history, exportable summaries, and time-off audits for your direct reports.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 print:hidden">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-sm font-medium rounded-xl transition shadow-lg shadow-indigo-600/20"
          >
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 print:hidden">
        <form onSubmit={handleSearchSubmit} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
          ...
          <div className="flex items-end gap-2">
            <button
              type="submit"
              className="flex-1 bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium py-2 px-4 rounded-xl transition shadow-md shadow-blue-600/20"
            >
              Filter
            </button>
            <button
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 col-span-2 sm:col-span-1">
          <span className="text-xs text-slate-400 font-medium">Reporting Scope</span>
          <div className="text-sm font-semibold text-slate-200 mt-1">Direct Reports Only</div>
        </div>
      </div>
              ...
              {loading ? (
                <tr>
                  <td colSpan="7" className="py-12 text-center text-slate-500">
                    ...
                    <span>Compiling team report...</span>
                  </td>
                </tr>
              ) : records.length === 0 ? (
                <tr>
                  <td colSpan="7" className="py-12 text-center text-slate-500">
                    ...
                    <p className="text-base font-medium text-slate-300">No records match the report criteria</p>
                    <p className="text-xs text-slate-500 mt-1">Try changing the year or filter parameters.</p>
                  </td>
                </tr>
              ) : (
                records.map((row) => (
