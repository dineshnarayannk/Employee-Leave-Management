...
const LEAVE_TYPE_COLORS = ['#6366F1', '#3B82F6', '#10B981', '#F59E0B', '#EC4899', '#8B5CF6'];
...
export default function LeaveAnalytics() {
  ...
  const fetchAnalytics = async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await getManagerAnalytics(year);
  ...
  const employeeUsage = data?.employeeUsage || [];
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900 border border-slate-800 rounded-2xl p-6 relative overflow-hidden">
        ...
        <div className="relative">
          ...
          <p className="text-slate-400 text-sm mt-1">
            Visual trends, leave distribution, and time-off patterns for your direct reports.
          </p>
        </div>
      ...
      {data && (
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 hover:border-slate-700 transition">
              ...
              <div className="text-xs text-slate-400 mt-1 flex items-center gap-1">
                ...
                <span className="font-semibold text-slate-300">{summary.teamDirectReportsCount}</span>
              </div>
            </div>
            ...
            <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 hover:border-slate-700 transition">
              ...
              <div className="text-xs text-slate-400 mt-1">
                Across {summary.approvedRequests} approved request(s)
              </div>
            </div>
            ...
            <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 hover:border-slate-700 transition">
              ...
              <div className="text-xs text-slate-400 mt-1 flex items-center gap-2">
                ...
                <span>•</span>
                <span className="text-rose-400 font-medium">{summary.rejectedRequests} Rejected</span>
              </div>
            </div>
            ...
            <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 hover:border-slate-700 transition">
              ...
              <div className="text-xs text-slate-400 mt-1">
                {summary.pendingRequests > 0 ? 'Requires your review' : 'All requests reviewed'}
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="text-base font-semibold text-white">Monthly Leave Days & Requests</h3>
                  <p className="text-xs text-slate-400 mt-0.5">Approved leave days and total volume in {year}</p>
                </div>
              </div>
              ...
              <div className="h-72 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={monthlyTrend} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    ...
                    <Bar dataKey="totalRequests" name="Total Requests" fill="#6366F1" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 flex flex-col">
              <div className="mb-2">
                ...
                <p className="text-xs text-slate-400 mt-0.5">Request outcomes for {year}</p>
              </div>
          ...
          {/* Charts Row 2: Leave Type Usage & Team Member Activity */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Leave Type Usage */}
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6">
              <div className="mb-4">
                <h3 className="text-base font-semibold text-white">Leave Category Utilization</h3>
                <p className="text-xs text-slate-400 mt-0.5">Distribution of requested leave categories</p>
              </div>

              {leaveTypeUsage.length === 0 ? (
                <div className="text-center py-10 text-slate-500 text-sm">No data available</div>
              ) : (
                <div className="h-64 w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      ...
                      />
                      <Legend wrapperStyle={{ fontSize: '12px' }} />
                      <Bar dataKey="totalRequests" name="Total Requests" fill="#8B5CF6" radius={[0, 4, 4, 0]} />
            ...
            {/* Direct Reports Leave Table */}
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 flex flex-col">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="text-base font-semibold text-white">Team Member Activity</h3>
                  <p className="text-xs text-slate-400 mt-0.5">Time-off frequency per direct report</p>
                </div>
              ...
              {employeeUsage.length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center text-slate-500 py-10">
                  <Users className="w-10 h-10 stroke-1 mb-2 text-slate-600" />
                  <p className="text-sm">No active direct reports assigned</p>
                </div>
              ) : (
                    ...
                    <tbody className="divide-y divide-slate-800/60">
                      {employeeUsage.map((emp) => (
                        <tr key={emp.employee_id} className="hover:bg-slate-800/40 transition">
                          ...
                          <td className="py-3 text-center font-semibold text-amber-400">
                            {emp.pendingRequests}
                          </td>
                        </tr>
                      ))}
